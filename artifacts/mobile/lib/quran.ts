...
export function arabicDigits(value: number) {
  return String(value).replace(/\d/g, (digit) => "٠١٢٣٤٥٦٧٨٩"[Number(digit)]);
}

export function pageSurah(page: number) {
  const index = Math.min(SURAHS.length - 1, Math.floor(((page - 1) / TOTAL_PAGES) * SURAHS.length));
  return SURAHS[index];
}
